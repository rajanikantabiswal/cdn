...
(function(global) {
  ...
  function TailwindSearchableDropdown(element, options) {
    ...
    this.options = Object.assign({
      ...
      classes: {
        ...
        selectedItem: 'flex items-center justify-between w-full px-3 py-2 text-sm bg-white border border-gray-300 rounded-md shadow-sm cursor-pointer focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500',
        dropdown: 'absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg',
        searchInput: 'w-full px-3 py-2 text-sm border-b border-gray-300 focus:outline-none focus:ring-blue-500 focus:border-blue-500',
        optionsContainer: 'max-h-60 overflow-y-auto py-1',
        option: 'px-3 py-2 text-sm cursor-pointer hover:bg-gray-100',
        selectedOption: 'px-3 py-2 text-sm cursor-pointer bg-blue-100',
  ...
  TailwindSearchableDropdown.prototype.init = function() {
    ...
    if (!this.element.classList.contains('tsd-container')) {
      ...
      container.className = 'tsd-container ' + this.options.classes.container;
  ...
  TailwindSearchableDropdown.prototype.render = function() {
    ...
    this.selectedEl.className = 'tsd-selected ' + this.options.classes.selectedItem;
    ...
    chevronSvg.setAttribute('class', this.options.classes.chevronIcon);
    ...
    this.dropdownEl.className = 'tsd-dropdown ' + this.options.classes.dropdown;
    ...
    this.searchEl.className = 'tsd-search ' + this.options.classes.searchInput;
    this.searchEl.placeholder = this.options.searchPlaceholder;
    this.dropdownEl.appendChild(this.searchEl);
    
    // Create options container
    this.optionsContainer = document.createElement('div');
    this.optionsContainer.className = 'tsd-options-container ' + this.options.classes.optionsContainer;
  ...
  TailwindSearchableDropdown.prototype.renderOptions = function() {
    ...
    if (this.options.data.length === 0) {
      ...
      noOption.className = 'tsd-option tsd-no-results ' + this.options.classes.noResults;
    ...
    this.options.data.forEach(item => {
      ...
      option.className = 'tsd-option ' + this.options.classes.option;
  ...
  TailwindSearchableDropdown.prototype.filterOptions = function(searchTerm) {
    ...
    if (!hasResults) {
      if (!noResults) {
        ...
        noResults.className = 'tsd-option tsd-no-results ' + this.options.classes.noResults;
  ...
  TailwindSearchableDropdown.prototype.select = function(value, label) {
    ...
    options.forEach(option => {
      if (option.getAttribute('data-value') === value) {
        option.className = 'tsd-option tsd-option-selected ' + this.options.classes.selectedOption;
      } else {
        option.className = 'tsd-option ' + this.options.classes.option;
      }
    });
  ...
  // Update classes
  TailwindSearchableDropdown.prototype.updateClasses = function(classOptions) {
    this.options.classes = Object.assign(this.options.classes, classOptions);
    this.render();
  };
